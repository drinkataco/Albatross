...
var _PushMenu = __webpack_require__(12);
...
(function (root, factory) {
  ...
  nodeListToArray = function nodeListToArray(nodeList) {
    var childItems = [];for (var i = 0, nll = nodeList[length]; i < nll; i++) {
  ...
  var Affix = function Affix(element, options) {
    ...
    getMaxScroll = function getMaxScroll() {
      ...
      if (self[target] !== null) {
        ...
        return parseInt(_typeof(self[offsetTop]) === fn ? self[offsetTop]() : self[offsetTop] || 0);
      }
    },
        getOffsetBottom = function getOffsetBottom() {
      if (self[offsetBottom]) {
        return maxScroll - element[offsetHeight] - parseInt(_typeof(self[offsetBottom]) === fn ? self[offsetBottom]() : self[offsetBottom] || 0);
      }
    },
      ...
      if (!affixedToBottom && !hasClass(element, affixBottom)) {
        ...
        unPinBottom = function unPinBottom() {
      if (affixedToBottom && hasClass(element, affixBottom)) {
  ...
  var Button = function Button(element, option) {
    ...
    if (hasClass(element, 'btn')) {
      ...
      for (var i = 0; i < lbll; i++) {
        !hasClass(labelsToACtivate[i], active) && queryElement('input', labelsToACtivate[i])[getAttribute](checked) && addClass(labelsToACtivate[i], active);
      }
  ...
  var Modal = function Modal(element, options) {
    ...
    getWindowWidth = function getWindowWidth() {
      ...
      overlay = queryElement('.' + modalBackdropString);
      if (overlay && overlay !== null && (typeof overlay === 'undefined' ? 'undefined' : _typeof(overlay)) === 'object') {
        modalOverlay = 0;
  ...
  var ScrollSpy = function ScrollSpy(element, options) {
    ...
    var updateItem = function updateItem(index) {
      var parent = items[index][parentNode],
          // item's parent LI element
  ...
  _createClass(BoxRefresh, [{
    ...
    value: function load() {
      ...
      var httpRequest = function httpRequest(resolve, reject) {
        // Create param string
        var params = '?';
        _this2.options.params = stringToObj(_this2.options.params);
        Object.keys(_this2.options.params).map(function (param) {
          params = '' + params + param + '=' + _this2.options.params[param] + '&';
          return params;
        });
        ...
        _this2.xhr.open('GET', '' + _this2.options.source + params);

        // Add headers
        _this2.options.headers = stringToObj(_this2.options.headers);
        Object.keys(_this2.options.headers).map(function (header) {
          return _this2.xhr.setRequestHeader(header, _this2.options.headers[header]);
        });

        _this2.xhr.onload = function () {
          if (_this2.xhr.status >= 200 && _this2.xhr.status < 300) {
            resolve(_this2.xhr.response);
          } else {
            reject(_this2.xhr.statusText);
          }
        };

        _this2.xhr.onerror = function () {
          return reject(_this2.xhr.statusText);
        };
        _this2.xhr.send();
      };
      ...
      var httpResolve = function httpResolve(response) {
        if (typeof _this2.options.onLoadDone === 'string') {
          window[_this2.options.onLoadDone](_this2, response);
        } else {
          _this2.options.onLoadDone.call(_this2, response);
        }

        // remove loading overlay if it was shown
        if (_this2.options.showOverlay) {
          _this2.removeOverlay();
        }

        if (_this2.options.loadInContent) {
          _this2.element.querySelector(_this2.options.content).innerHTML = response;
        }
      };
      ...
      var request = new Promise(function (resolve, reject) {
        if (typeof _this2.options.onLoadStart === 'string') {
          window[_this2.options.onLoadStart](_this2, reject, resolve);
        } else {
          _this2.options.onLoadStart.call(_this2, reject, resolve);
        }
      });
  ...
  _createClass(BoxWidget, [{
    ...
    value: function remove() {
      ...
      if (typeof Velocity === 'undefined') {
        ...
        Velocity(this.element, 'slideUp', {
          ...
          _this2.element.dispatchEvent(new CustomEvent(_this2.Event.removed));
    ...
    value: function collapse() {
      ...
      Array.prototype.forEach.call(collapseIcons, function (i) {
        i.classList.remove(_this3.options.collapseIcon);
        i.classList.add(_this3.options.expandIcon);
      });

      // Determine whether to dispatch collapsed event
      var dispatchEvent = function dispatchEvent(fireEvent) {
        if (fireEvent) {
          _this3.element.dispatchEvent(new CustomEvent(_this3.Event.collapsed));
        }
      };
        ...
        if (boxElement) {
          ...
          if (typeof Velocity === 'undefined') {
            ...
            Velocity(boxElement, 'slideUp', {
              easing: _this3.options.easing,
              duration: _this3.options.animationSpeed
            }).then(function () {
              dispatchEvent(fireEvent);
            });
          }
      ...
      setTimeout(function () {
        return _this3.element.classList.add(_this3.ClassName.collapsed);
    ...
    value: function expand() {
      ...
      Array.prototype.forEach.call(collapseIcons, function (i) {
        i.classList.remove(_this4.options.expandIcon);
        i.classList.add(_this4.options.collapseIcon);
      });

      // Determine whether to dispatch expanded event
      var dispatchEvent = function dispatchEvent(fireEvent) {
        if (fireEvent) {
          _this4.element.dispatchEvent(new CustomEvent(_this4.Event.expanded));
        }
      };
        ...
        if (boxElement) {
          ...
          if (typeof Velocity === 'undefined') {
            ...
            Velocity(element, 'slideDown', {
              ...
              duration: _this4.options.animationSpeed
            }).then(function () {
              dispatchEvent(fireEvent);
            });
          }
      ...
      // Add collapsed class after animation finished
      setTimeout(function () {
        return _this4.element.classList.remove(_this4.ClassName.collapsed);
      }, this.options.animationSpeed);
    }
  }]);
...
var Layout = function () {
  ...
  _createClass(Layout, [{
    ...
    value: function activate() {
      ...
      if (sidebarMenu) {
        sidebarMenu.addEventListener(_Tree2.default.Event.expanded, this.fixLayout.bind(this));
        sidebarMenu.addEventListener(_Tree2.default.Event.collapsed, this.fixLayout.bind(this));
      }
    }
...
var PushMenu = function () {
  ...
  function PushMenu(element, options, classNames, selectors) {
    ...
    this.body.getElementsByClassName(this.ClassName.contentWrapper)[0].addEventListener('click', function () {
      if (_this.windowWidth <= _this.options.collapseScreenSize && _this.body.classList.contains(_this.ClassName.open)) {
        _this.close();
      }
    });
  ...
  _createClass(PushMenu, [{
    ...
    value: function expand() {
      ...
      window.setTimeout(function () {
        _this3.body.classList.remove(_this3.ClassName.collapsed);
        _this3.body.classList.add(_this3.ClassName.expanded);
    ...
    value: function collapse() {
      ...
      window.setTimeout(function () {
        _this4.body.classList.remove(_this4.ClassName.expanded);
        _this4.body.classList.add(_this4.ClassName.collapsed);
      }, this.options.expandTransitionDelay);
    }
    ...
    value: function expandOnHover() {
      ...
      Array.prototype.forEach.call(document.getElementsByClassName(this.ClassName.mainSidebar), function (context) {
        context.addEventListener('mouseover', function () {
          // Handle Expansion
          if (_this5.body.classList.contains(_this5.ClassName.mini) && _this5.body.classList.contains(_this5.ClassName.collapsed) && _this5.windowWidth > _this5.options.collapseScreenSize) {
            _this5.expand();
          }
        });
        ...
        context.addEventListener('mouseleave', function () {
          if (_this5.body.classList.contains(_this5.ClassName.expanded)) {
            _this5.collapse();
          }
        });
      });
